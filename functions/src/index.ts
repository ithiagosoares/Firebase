...
export const onPatientAppointmentUpdate = onDocumentUpdated(
  ...
  async (event) => {
    ...
    if (
      before.nextAppointment?.toMillis() === after.nextAppointment?.toMillis()
    )
      return;
    const userId = event.params.userId;
...
export const sendScheduledMessages = onSchedule(
  ...
  async () => {
    ...
    logger.info(
      `Disparando ${messagesToSend.docs.length} mensagens via Cloud Run...`
    );
...
async function processScheduledMessage(
  ...
  try {
    ...
    if (!url)
      throw new Error(
        "WHATSAPP_API_URL não está configurada nos parâmetros de ambiente."
      );
    ...
    logger.info(`✅ Mensagem para ${message.patientPhone} delegada.`);
  } catch (error: any) {
    const errorMessage = error.response?.data?.error || error.message;
    logger.error(
      `❌ Falha no envio para ${message.patientPhone}: ${errorMessage}`
    );
...
export const createStripeCustomer = onCall(
  { region: "southamerica-east1", secrets: [stripeSecretKey] },
...
export const createCheckoutSession = onCall(
  ...
  async (req: CallableRequest) => {
    ...
    const stripeId = (await db.doc(`users/${req.auth.uid}`).get()).data()
      ?.stripeId;
    if (!stripeId)
...
export const createStripePortalSession = onCall(
  ...
  async (req: CallableRequest) => {
    if (!req.auth?.uid)
      throw new HttpsError("unauthenticated", "URL de retorno obrigatória.");
